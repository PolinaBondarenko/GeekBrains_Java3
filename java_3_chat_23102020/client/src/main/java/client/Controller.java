...
public class Controller implements Initializable {
    ...
    private File logFile = new File("D:\\MyJavaProjects\\" +
            "java_3_chat_23102020\\client\\src\\main\\resources\\chatLog.txt");
    ...
    private void showHistory() {
        ...
        try(BufferedReader reader = new BufferedReader(new FileReader(logFile))) {
            reader.lines().forEach(history::add);
            if (history.size() > 100) {
                for (int i = 0; i < (history.size() - 100); i++) {
                    history.remove(i);
                }
                history.forEach(line -> textArea.appendText(line + "\n"));
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void sendMsg(ActionEvent actionEvent) {
        ...
        try (FileWriter fileWriter = new FileWriter(logFile, true)){
            ...
            fileWriter.append( nickname + " : " +textField.getText() + "\n");
            textField.clear();
